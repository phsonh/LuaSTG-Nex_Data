...
function M.IsOutWorld(x, y, margin)
    margin = margin or 0

    return x < world.boundl - margin
        or x > world.boundr + margin
        or y < world.boundb - margin
        or y > world.boundt + margin
end

function M.IsInWorld(x, y, margin)
    return not M.IsOutWorld(x, y, margin)
end
...
function M.SetViewMode(mode)
    ...
    error("View.SetViewMode(mode): invalid mode '" .. tostring(mode) .. "'", 2)
end
